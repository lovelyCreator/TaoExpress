...
import { Product, UseProductDetailMutationOptions, UseProductDetailMutationResult } from '../types';

export const useProductDetailMutation = (options?: UseProductDetailMutationOptions): UseProductDetailMutationResult => {
  ...
  const mutate = useCallback(async (productId: string) => {
    setIsLoading(true);
    setIsSuccess(false);
    setIsError(false);
    setError(null);
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log("After receiving ApiProduct Data from api: ", apiProduct);
        ...
        console.log("After receiving Variations Data from api: ", response.data.price);
        ...
        const productData: Product = {
          ...
          rating_count: 0,
          wishlists_count: apiProduct.whislists_count || 0,
        };
        console.log("After receiving Product Data from api: ", productData);
        setData(productData);
        setIsSuccess(true);
